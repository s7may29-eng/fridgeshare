...
const BOX_ICON_KEYS = ['fridge','freezer','condiment','stock','alcohol','coffee','bath','laundry','electronics','medicine','gym','other'];
const BOX_LABELS = { fridge:'冷蔵庫', freezer:'冷凍庫', condiment:'調味料', stock:'ストック食材', alcohol:'お酒', coffee:'コーヒー・お茶', bath:'洗面・お風呂', laundry:'洗濯・掃除', electronics:'家電関連', medicine:'薬・衛生', gym:'ジム用品', other:'その他' };
          ...
          <div style={{display:'flex', gap:6, flexWrap:'wrap', marginTop:8, marginBottom:16}}>
            {BOX_ICON_KEYS.map(k=>(
              <button key={k} onClick={()=>setForm(p=>({...p,boxIcon:k}))}
                style={{background:form.boxIcon===k?accentLight:'#f5f5f3', border:form.boxIcon===k?'1.5px solid '+accent:'1.5px solid transparent', borderRadius:12, padding:'8px 6px', cursor:'pointer', transition:'all 0.15s', display:'flex', flexDirection:'column', alignItems:'center', gap:4, minWidth:58}}>
                <BoxIcon k={k} size={36} />
