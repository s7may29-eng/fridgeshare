...
// ============================================================
// STORAGE — localStorage (works in any browser / PWA)
// ============================================================
const KEYS = {
USERS: “fs-users”,
FRIDGES: “fs-fridges”,
...
const handleLogout = () => {
setSession(null); setCurrentFridge(null); save(KEYS.SESSION, null); setScreen(“auth”);
};
...
const deleteItem = (id) => {
const ni = { …items }; delete ni[id];
persist(null, null, ni); setConfirmDelete(null); showToast(“削除しました”, “info”);
};
...
// STYLES
const S = {
app: { minHeight: “100dvh”, background: “linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #0f2a1e 100%)”, fontFamily: “‘Noto Sans JP’, sans-serif”, color: “#e2e8f0”, position: “relative”, overflow: “hidden” },
...
btn: (c = “#22c55e”) => ({ background: `linear-gradient(135deg,${c},${c}cc)`, color: “#fff”, border: “none”, borderRadius: 12, padding: “13px 24px”, fontSize: 15, fontWeight: 700, cursor: “pointer”, width: “100%”, marginTop: 8, boxShadow: `0 4px 20px ${c}44`, transition: “opacity .2s” }),
...
toastEl: (t) => ({ position: “fixed”, bottom: 80, left: “50%”, transform: “translateX(-50%)”, background: t === “error” ? “#ef4444” : t === “success” ? “#22c55e” : “#3b82f6”, color: “#fff”, padding: “12px 20px”, borderRadius: 12, fontSize: 14, fontWeight: 600, zIndex: 1000, whiteSpace: “nowrap”, boxShadow: “0 8px 30px rgba(0,0,0,.4)” }),
};

const CSS = `@keyframes su{from{opacity:0;transform:translateX(-50%) translateY(20px)}to{opacity:1;transform:translateX(-50%) translateY(0)}} *{box-sizing:border-box} select option{background:#1e293b} input[type=date]{color-scheme:dark} .ic:hover{background:rgba(255,255,255,.08)!important}`;

// ── LOADING ──
if (screen === “loading”) return (
<div style={{ …S.app, display: “flex”, alignItems: “center”, justifyContent: “center” }}>
<div style={{ textAlign: “center” }}><div style={{ fontSize: 64 }}>🧊</div><div style={{ color: “#64748b”, marginTop: 12 }}>読み込み中…</div></div>
</div>
);

// ── AUTH ──
if (screen === “auth”) return (
<div style={S.app}><div style={S.grain} /><style>{CSS}</style>
<div style={S.wrap}>
<div style={{ paddingTop: 60, paddingBottom: 40, textAlign: “center” }}>
<div style={{ fontSize: 64, marginBottom: 12 }}>🧊</div>
<h1 style={{ fontSize: 28, fontWeight: 900, margin: 0, background: “linear-gradient(135deg,#22c55e,#60a5fa)”, WebkitBackgroundClip: “text”, WebkitTextFillColor: “transparent” }}>FridgeShare</h1>
...
style={{ flex: 1, padding: 10, borderRadius: 10, border: “none”, cursor: “pointer”, fontWeight: 700, fontSize: 14, background: authMode === m ? “rgba(34,197,94,.2)” : “transparent”, color: authMode === m ? “#22c55e” : “#64748b” }}>
{m === “login” ? “ログイン” : “新規登録”}
</button>
))}
</div>
...
<div style={{ marginBottom: 20 }}><label style={S.label}>パスワード{authMode === “register” && “ (6文字以上)”}</label><input style={S.input} type=“password” placeholder=”••••••••” value={form.password || “”} onChange={e => setForm(p => ({ …p, password: e.target.value }))} onKeyDown={e => e.key === “Enter” && (authMode === “login” ? handleLogin() : handleRegister())} /></div>
...
{toast && <div style={{ …S.toastEl(toast.type), animation: “su .3s ease” }}>{toast.msg}</div>}
</div>
);
...
{Object.values(fridges).filter(f => f.members.includes(session.userId)).length > 0 && (
<div style={{ marginBottom: 24 }}>
<div style={{ …S.label, marginBottom: 10 }}>参加中の冷蔵庫</div>
...
return (
<div key={f.id} onClick={() => { const s = { …session, fridgeId: f.id }; setSession(s); save(KEYS.SESSION, s); setCurrentFridge(f.id); setScreen(“fridge”); }}
style={{ …S.card, marginBottom: 8, cursor: “pointer”, display: “flex”, alignItems: “center”, justifyContent: “space-between” }}>
<div style={{ display: “flex”, alignItems: “center”, gap: 12 }}>
<div style={{ fontSize: 32 }}>🧊</div>
<div><div style={{ fontWeight: 700, fontSize: 16 }}>{f.name}</div><div style={{ color: “#64748b”, fontSize: 12 }}>メンバー {f.members.length}人 · {count}品</div></div>
</div>
...
<input style={S.input} placeholder=“冷蔵庫の名前（例：リビングの冷蔵庫）” value={form.fridgeName || “”} onChange={e => setForm(p => ({ …p, fridgeName: e.target.value }))} />
...
{showInvite && (
<div style={{ …S.card, marginBottom: 16, borderColor: “rgba(34,197,94,.3)” }}>
<div style={{ fontSize: 12, color: “#64748b”, marginBottom: 8, fontWeight: 600 }}>招待コード</div>
<div style={{ background: “rgba(34,197,94,.1)”, borderRadius: 10, padding: “12px 16px”, fontFamily: “monospace”, fontSize: 18, fontWeight: 900, color: “#22c55e”, letterSpacing: 2, textAlign: “center” }}>{fridge?.inviteCode}</div>
<div style={{ fontSize: 12, color: “#64748b”, marginTop: 8, textAlign: “center” }}>このコードを共有相手に教えてください</div>
{fridge?.ownerId === session.userId && <button style={S.ghost} onClick={refreshInviteCode}>コードを再発行（旧コード無効化）</button>}
</div>
)}
{expiredItems.length > 0 && <div style={{ background: “rgba(239,68,68,.15)”, border: “1px solid rgba(239,68,68,.3)”, borderRadius: 12, padding: “10px 14px”, marginBottom: 10, fontSize: 13 }}>⚠️ 期限切れ {expiredItems.length}品 — 確認してください</div>}
...
style={{ whiteSpace: “nowrap”, padding: “6px 12px”, borderRadius: 20, border: “none”, cursor: “pointer”, fontSize: 12, fontWeight: 600, background: filterCat === cat ? “rgba(34,197,94,.25)” : “rgba(255,255,255,.06)”, color: filterCat === cat ? “#22c55e” : “#64748b” }}>
{cat === “all” ? “すべて” : `${CAT_ICONS[cat]} ${cat}`}
</button>
))}
</div>
<div style={{ display: “flex”, gap: 8, marginBottom: 16, alignItems: “center” }}>
<span style={{ fontSize: 12, color: “#64748b”, whiteSpace: “nowrap” }}>並び替え:</span>
...
? <div style={{ textAlign: “center”, padding: “40px 0”, color: “#64748b” }}><div style={{ fontSize: 48, marginBottom: 12 }}>📭</div><div>食材がありません</div></div>
: filteredItems.map(item => {
const exp = isExpired(item.expiry), expi = isExpiringSoon(item.expiry);
const byUser = users[item.addedBy];
return (
<div key={item.id} className=“ic” style={{ background: “rgba(255,255,255,.04)”, border: `1px solid ${CAT_COLORS[item.category] || "#374151"}33`, borderRadius: 16, padding: “14px 16px”, marginBottom: 8, display: “flex”, alignItems: “center”, gap: 12, cursor: “pointer” }} onClick={() => openEditItem(item)}>
...
<span style={S.badge(CAT_COLORS[item.category])}>{item.category}</span>
...
<button onClick={e => { e.stopPropagation(); setConfirmDelete(item); }} style={{ background: “rgba(239,68,68,.15)”, border: “1px solid rgba(239,68,68,.2)”, color: “#f87171”, borderRadius: 8, padding: “6px 10px”, cursor: “pointer”, fontSize: 14, flexShrink: 0 }}>🗑</button>
</div>
);
})}
<div style={{ height: 90 }} />
</div>
<button onClick={() => { setForm({}); setScreen(“addItem”); }}
style={{ position: “fixed”, bottom: 24, right: 24, width: 60, height: 60, borderRadius: “50%”, background: “linear-gradient(135deg,#22c55e,#16a34a)”, border: “none”, color: “#fff”, fontSize: 28, cursor: “pointer”, boxShadow: “0 8px 30px rgba(34,197,94,.5)”, zIndex: 100, display: “flex”, alignItems: “center”, justifyContent: “center” }}>+</button>
{confirmDelete && (
<div style={{ position: “fixed”, inset: 0, background: “rgba(0,0,0,.7)”, backdropFilter: “blur(4px)”, zIndex: 200, display: “flex”, alignItems: “center”, justifyContent: “center”, padding: “0 20px” }}>
<div style={{ …S.card, maxWidth: 340, width: “100%” }}>
<div style={{ fontSize: 36, textAlign: “center”, marginBottom: 12 }}>🗑️</div>
<div style={{ fontWeight: 700, textAlign: “center”, marginBottom: 8 }}>「{confirmDelete.name}」を削除しますか？</div>
<div style={{ color: “#64748b”, fontSize: 13, textAlign: “center”, marginBottom: 16 }}>この操作は元に戻せません</div>
<button style={S.btn(”#ef4444”)} onClick={() => deleteItem(confirmDelete.id)}>削除する</button>
<button style={S.ghost} onClick={() => setConfirmDelete(null)}>キャンセル</button>
</div>
</div>
)}
{toast && <div style={{ …S.toastEl(toast.type), animation: “su .3s ease” }}>{toast.msg}</div>}
...
<div style={S.card}>
<div style={{ marginBottom: 16 }}><label style={S.label}>品名 *</label><input style={S.input} placeholder=“例：牛乳” value={form.itemName || “”} onChange={e => setForm(p => ({ …p, itemName: e.target.value }))} autoFocus /></div>
